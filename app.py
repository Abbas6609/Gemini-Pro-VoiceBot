...
# Define a dictionary of hardcoded questions and answers
predefined_qa = {
    "can you introduce yourself briefly": "I am Gemini-Pro model powered by Google. How can I assist you?",
...
# Modified function to use Gemini model for response generation with different configurations for voice and text
def generate_response(user_input, for_voice=False):
    normalized_input = user_input.lower()
    if normalized_input in predefined_qa:
        st.write("Using Gemini model for response.")
        return predefined_qa[normalized_input]
    ...
    try:
        ...
        if for_voice:
            ...
            generation_config = genai.types.GenerationConfig(
                ...
                temperature=1.0)
            response = model.generate_content(normalized_input, generation_config=generation_config)
        else:
            # Default configuration for text response
            response = model.generate_content(normalized_input)
